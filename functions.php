 ...
 *
 * La struttura dell'array � la seguente:
 *	id - id della lingua
 *	cml_default - 1 o 0 a seconda se la lingua � quella predefinita o meno
 ...
 *	cml_category_name - nome della categoria a cui � collegata la lingua
...
/**
 * Restituisco il men� da utilizzare in base alla lingua "attuale"
 */
function cml_get_menu() {
  //Restituisco il nome del men� da utilizzare a seconda della lingua
...
/**
 * Controllo se la lingua attuale � quella di default
 *
 * @return True/False a seconda se la lingua corrente � quella impostata come predefinita
 */
 ...
 * @param "echo" - indica se false non esegue il costrutto echo ma ritorner� la lista <ul> che ho creato
 * @param "linked" - se true: la bandiera deve restituire il link all'articolo nelle varie lingue (se presente),
* 				false: la bandiera punter� alla home aggiungendo il suffisso "?lang=##"
 */
...
/**
 * restiutisco l'avviso che l'articolo � disponibile nella lingua specificata
 *
...
function cml_is_homepage() {
  ...
  //Controllo se � stata impostata una pagina "statica" se l'id di questa � = a quello della statica
...
function cml_get_page_id_by_path($url, $types = null) {
  ...
  //Se l'ultimo elemento � vuoto, lo cancello ;)
...
function cml_get_the_link( $result, $linked = true, $only_existings = false ) {
  ...
  if( cml_is_homepage() && ! in_the_loop() ) {
    //Se st� nella home vuol dire che ho scelto come metodo di reindirizzamento &lang
    ...
    if( is_category() ) :
      ...
      if( is_array( $cat ) ) :
        ...
        //Mi recuperer� il link tradotto dal mio plugin ;)
    ...
    /* Controllo se � stata impostata una pagina statica,
	perch� cos� invece di restituire il link dell'articolo collegato
	aggiungo il pi� "bello" ?lang=## alla fine della home.

	Se non ho trovato nesuna traduzione per l'articolo, la bandiera punter� alla homepage
    */
